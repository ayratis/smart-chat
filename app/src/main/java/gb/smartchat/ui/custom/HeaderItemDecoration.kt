...
class HeaderItemDecoration(
    ...
    private fun isHeader(itemPosition: Int) =
        (parent.adapter as? StickyHeaderHelper)?.isHeaderItem(itemPosition) ?: false
    ...
    private val bg = ColorDrawable(parent.context.color(R.color.white))
    private fun drawHeader(c: Canvas, header: View, childInContact: View, paddingTop: Int) {
        c.save()
        bg.setBounds(0, 0, c.width, childInContact.top)
        bg.draw(c)
        c.translate(0f, paddingTop.toFloat())
        header.draw(c)
        c.restore()
    ...
    private fun moveHeader(c: Canvas, currentHeader: View, inContact: View, paddingTop: Int) {
        c.save()
        if (!shouldFadeOutHeader) {
            ...
            if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.LOLLIPOP) {
                c.saveLayerAlpha(
                    ...
                    (((inContact.top - paddingTop) / inContact.height.toFloat()) * 255).toInt()
                )
            } else {
        ...
        c.translate(0f, (inContact.top - currentHeader.height).toFloat() /*+ paddingTop*/)
